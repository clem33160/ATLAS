...
from .common import RUNTIME_PATHS, new_id, now_iso
...
def ingest_rapporteur_run(summary_path=None, rejected_path=None, leads_path=None):
    ...
    if summary_p.exists():
        ...
        is_dry_run = str(payload.get('mode', '')).lower() == 'dry-run'
        source_tag = 'demo_fixture' if is_dry_run else 'system'
        create_system_event('rapporteur_ingest', 'rapporteur_affaires', 'summary ingested', 0.9, {**payload, 'source_tag': source_tag})
    ...
    if rejected_p.exists():
        ...
        for item in rejected[:20]:
            ...
            if "CONFLICT" in reason:
                ...
                target_domain = 'rapporteur_affaires_demo' if is_dry_run else 'rapporteur_affaires'
                uncertainty_store.add_uncertainty(target_domain, item.get("title", "unknown"), reason or "needs review", 0.4)
